...
describe ('Resources Actions', () => {
    it ('returns load resources action', () => {
        ...
        expect(response).toStrictEqual({ type: LOAD_RESOURCES, isLoading: false, isError: false, resources: [{ id: 1 }]});
    });

    it ('returns save donors action', () => {
        const response = saveResource({ id: 1 });
        expect(response).toStrictEqual({ type: SAVE_RESOURCE, resource: { id: 1 }});
    });
});